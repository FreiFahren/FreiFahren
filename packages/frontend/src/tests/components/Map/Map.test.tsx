...
const mockGeolocationScript =
    'navigator.geolocation.getCurrentPosition = success =>  success({ coords: { latitude: 52.5200, longitude: 13.4050, }, timestamp: Date.now() });';
...
testcafeTest(
    'FreifahrenMap renders the map with the correct initial view state',
    async (t) => {
        const map = Selector('#map');
...
testcafeTest(
    'FreifahrenMap does not render LocationMarker when userPosition is null',
    async (t) => {
