...
export const redefineLineOptions = (
    option: selectOption,
    ...
    const newLineOptions = Object.entries(stationsList)
        ...
        .flatMap(([, station]) =>
            station.lines.map((line) => ({ value: line, label: line }))
        );
...
export const redefineStationOptions = (
    ...
    for (const station_id of linesList[option.value]) {
        newStationOptions.push({
            value: station_id,
            label: stationsList[station_id].name,
        });
    }
...
export const redefineDirectionOptions = (
    ...
    const { 0: firstStationId, [length - 1]: lastStationId } =
        linesList[option.value];
