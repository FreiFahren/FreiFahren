...
export const ViewedReportsProvider = ({ children }: PropsWithChildren) => {
    const [reports, setReports] = useState<ViewedReport[]>(loadInitialReports)
    ...
    const isRecentAndUnviewed = useCallback(
        (report: MarkerData): boolean => isReportRecent(report) && !hasViewedReport(report),
        [hasViewedReport]
    )
