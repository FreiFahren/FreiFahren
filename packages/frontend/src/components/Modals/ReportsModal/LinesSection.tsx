...
interface LinesSectionProps {
    getChartData: { line: string; reports: number; displayLine?: string }[]
}
...
const CustomBarShape = ({ x, y, width, height, payload }: any) => {
    const color = getLineColor(payload.displayLine ?? payload.line)
...
const LinesSection: React.FC<LinesSectionProps> = ({ getChartData }) => {
    ...
    const chartData = Array.from(
        getChartData
            .map((item) => ({
                ...item,
                displayLine: lines?.find((line) => line.id === item.line)?.name ?? item.line,
            }))
            .reduce<Map<string, { line: string; reports: number; displayLine: string }>>((mergedData, item) => {
                const existingItem = mergedData.get(item.displayLine)
                mergedData.set(item.displayLine, {
                    line: existingItem?.line ?? item.line,
                    displayLine: item.displayLine,
                    reports: (existingItem?.reports ?? 0) + item.reports,
                })
                return mergedData
            }, new Map())
            .values()
    ...
    return (
        <section className="list-modal">
            ...
            <ResponsiveContainer width="100%" height={chartData.length * (34 + 12)}>
                <BarChart data={chartData} layout="vertical">
                    ...
                    />
                    <Tooltip content={<CustomTooltip getChartData={chartData} />} />
                    <Bar
                        dataKey="reports"
                        barSize={24}
                        radius={[4, 4, 4, 4]}
