...
export const berlinViewPosition: { lng: number; lat: number } = {
    lng: 13.388,
    lat: 52.5162,
};
...
const FreifahrenMap: React.FC<FreifahrenMapProps> = ({
    ...
    const SouthWestBounds: LngLatLike = {
        lng: 12.8364646484805,
        lat: 52.23115511676795,
    };
    const NorthEastBounds: LngLatLike = {
        lng: 13.88044556529124,
        lat: 52.77063424239867,
    };
        ...
        if (currentMap) {
            ...
            const intervalId = setInterval(() => {
                if (currentMap.isStyleLoaded()) {
                    ...
                    const moveLayerSafely = (
                        layerId: string,
                        beforeId: string
                    ) => {
                        if (
                            currentMap.getLayer(layerId) &&
                            currentMap.getLayer(beforeId)
                        ) {
                            currentMap.moveLayer(layerId, beforeId);
                        }
                    };

                    const lineLayerId = isRiskLayerOpen
                        ? 'risk-line-layer'
                        : 'line-layer';
                    const labelLayerId = isRiskLayerOpen
                        ? 'risk-label-layer'
                        : 'label-layer';
                ...
                <Suspense fallback={<div>Loading...</div>}>
                    ...
                    {isRiskLayerOpen ? (
                        <RiskLineLayer
                            ...
                            linesGeoJSON={
                                linesGeoJSON as GeoJSON.FeatureCollection<GeoJSON.LineString>
                            }
                            textColor={textColor}
                        />
                    ) : (
