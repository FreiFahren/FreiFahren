...
const MarkerContainer: React.FC<MarkersProps> = ({
    formSubmitted,
    isFirstOpen,
    userPosition,
}) => {
    const [ticketInspectorList, setTicketInspectorList] = useState<
        MarkerData[]
    >([]);
            ...
            if (newTicketInspectorList.length > 0) {
                setTicketInspectorList((currentList) => {
                    ...
                    const updatedList = new Map(
                        currentList.map((inspector) => [
                            inspector.station.id,
                            inspector,
                        ])
                    );

                    newTicketInspectorList.forEach(
                        (newInspector: MarkerData) => {
                            const existingInspector = updatedList.get(
                                newInspector.station.id
                            );
                            if (existingInspector) {
                                // Compare timestamps and wether it is historic to decide if we need to update
                                if (
                                    new Date(newInspector.timestamp) >=
                                        new Date(existingInspector.timestamp) &&
                    ...
                    lastReceivedInspectorTime.current = new Date(
                        newTicketInspectorList[0].timestamp
                    );
