...
details_corrected = details_ext %>% as_tibble() %>%
  group_by(line) %>%
  mutate(geometry = ifelse(line == "S41" & r %in% c(max(r),min(r)),list(ring_base),geometry)) %>%
  st_as_sf(crs=4326)

curves_ext = details_corrected %>%
  group_by(line) %>%
  summarise(do_union=FALSE) %>%
  st_cast("LINESTRING")


colors = readRDS("linecolors.RDS")
...
segments = stations_nf %>%
  ...
  nest_join(details_corrected, join_by(line, r_from <= r, r_to >= r), name = "details") %>%
  rowwise() %>%
  mutate(geometry = details %>% summarise(do_union=FALSE) %>% st_cast("LINESTRING") %$% geometry) %>%
  select(-details, -r_from, -r_to, -line_color) %>%
  ungroup() %>%
  left_join(colors) %>%
