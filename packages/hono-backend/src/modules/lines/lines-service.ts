...
export class LinesService {
    ...
    async isStationOnLine(stationId: string, lineId: string) {
        const result = await this.db
            ...
            .from(lineStations)
            .where(and(eq(lineStations.stationId, stationId), eq(lineStations.lineId, lineId)))

        return result.length > 0
    }

    async isLineCircular(lineId: string) {
        const result = await this.db.select({ isCircular: lines.isCircular }).from(lines).where(eq(lines.id, lineId))
