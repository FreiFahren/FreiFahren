...
export class ReportsService {
    ...
    async createReport(reportData: InsertReport) {
        ...
        const result = await this.db.insert(reports).values(postprocessed)

        return result
    }

    private async postprocessReport(reportData: InsertReport) {
        return [this.postprocessLineId.bind(this), this.postprocessDirectionId.bind(this)].reduce(
    ...
    private async postprocessLineId(reportData: InsertReport): Promise<InsertReport> {
        ...
        return {
            ...reportData,
            lineId: (await this.linesService.isStationOnLine(reportData.stationId, reportData.lineId))
                ? reportData.lineId
                : null,
        }
    }
    ...
    private async postprocessDirectionId(reportData: InsertReport): Promise<InsertReport> {
        ...
        return {
            ...reportData,
            directionId: (await this.linesService.isValidDirectionForLine(reportData.lineId, reportData.directionId))
